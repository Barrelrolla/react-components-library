...
export default <Record<string, CssInJs>>{
  ".navbar": {
    "@apply flex w-full bg-(--bg-color) text-(--fg-color) transition-colors duration-(--dropdown-animation-duration) left-0 z-(--z-navbar)":
      {},
  },
  ".navbar-backdrop": {
  ...
  ".navbar-link": {
    "@apply flex hover:bg-(--fg-color)/10 focus-visible:bg-(--fg-color)/10 active:bg-(--fg-color)/30 selected:bg-(--fg-color) selected:text-(--bg-color) selected:hover:bg-(--fg-color)/90 selected:focus-visible:bg-(--fg-color)/90 selected:active:bg-(--fg-color)/80 outline-offset-1 px-1 py-3":
      {},
  },
